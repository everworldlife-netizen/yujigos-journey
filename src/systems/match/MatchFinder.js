...
export default class MatchFinder {
  static find(grid) {
    ...
    horizontalRuns.forEach((run) => {
      if (run.length >= 4) specials.push({ row: run.row, col: run.start + Math.floor((run.length - 1) / 2), specialType: run.length >= 5 ? 'rainbow' : 'striped' });
    });
    verticalRuns.forEach((run) => {
      if (run.length >= 4) specials.push({ row: run.start + Math.floor((run.length - 1) / 2), col: run.col, specialType: run.length >= 5 ? 'rainbow' : 'bomb' });
    });

    const crosses = new Set();
    horizontalRuns.forEach((h) => {
      verticalRuns.forEach((v) => {
        if (v.col >= h.start && v.col < h.start + h.length && h.row >= v.start && h.row < v.start + v.length) crosses.add(`${h.row},${v.col}`);
      });
    });
    crosses.forEach((key) => {
    ...
    return {
      ...
      specials: Array.from(new Map(specials.map((s) => [`${s.row},${s.col}`, s])).values())
    };
  }

  // EXTENSION: Add special match patterns here
}
